...
TOTAL8_WITH_DOT_REGEX = re.compile(r"(?=(\d{1,7}\.\d{1,7}))")
...
def parse_mass_from_text(text):
    ...
    # 先找“总数字位数=8，且中间已有小数点”的候选
    best_token = None
    for m in TOTAL8_WITH_DOT_REGEX.finditer(s):
        token = m.group(1)
        digits_only = re.sub(r"\D", "", token)
        if len(digits_only) == 8 and token.count('.') == 1:
            best_token = token
            break
    if best_token is not None:
        try:
            return best_token, float(best_token)
        except Exception:
            pass

    digits = re.sub(r"\D", "", s)
    # 没读到小数点，但总数字位数够 8 位时，保留前 8 位并默认在中间偏合理位置试探
    # 优先尝试 4+4，再尝试 6+2 / 5+3 / 3+5，只接受总数字位数=8
    if len(digits) >= 8:
        digits8 = digits[:8]
        layouts = [4, 6, 5, 3, 2, 1, 7]
        for pos in layouts:
            token = digits8[:pos] + '.' + digits8[pos:]
            try:
                value = float(token)
                return token, value
            except Exception:
...
def recognize_multi(recognizer, crop_bgr):
    ...
    for v in variants:
        ...
        if norm and (norm is not None and len(re.sub(r'\D','', norm)) == 8 and norm.count('.') == 1):
...
def read_full_mass_near_peak(img_bgr, plot_box, peak_x, peak_y, recognizer):
    ...
    right = min(x2, peak_x + 190)
    top = max(0, int(peak_y) - 34)
    bottom = min(H, int(peak_y) + 12)
    ...
    for g in groups:
        ...
        pad_l, pad_r, pad_t, pad_b = 6, 34, 5, 5
        x0 = max(0, gx0 - pad_l)
        y0 = max(0, gy0 - pad_t)
    ...
    def score_candidate(c):
        ...
        short_penalty = 0 if (c.get('text') is not None and len(re.sub(r'\D','', str(c['text']))) == 8 and str(c['text']).count('.') == 1) else 80
...
def read_peak_label(img_bgr, plot_box, peak_x, peak_y, detector, recognizer):
    # 先用“总数字位数=8，小数点位置不确定”规则专门读取最高峰旁边整段文本
    ...
    if full is not None and full.get("text"):
        ...
        full["value"] = value
        if (full.get('text') is not None and len(re.sub(r'\D','', str(full['text']))) == 8 and str(full['text']).count('.') == 1):
            return full
...
def main(image_path=IMAGE_PATH, show_figure=SHOW_FIGURE):
    ...
    if show_figure:
        ...
        plt.axis("off")
        plt.title("Highest peak + label value (PP-OCR det+rec ONNX, fixed total-8-digit mass format)")
        plt.tight_layout()
        plt.show()
